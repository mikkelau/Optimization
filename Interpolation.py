# -*- coding: utf-8 -*-
"""
Created on Fri Aug 22 16:27:31 2025
...
def interpolate(alpha1, alpha2, f1, f2, slope1, slope2, interpolation='cubic'):
    
    if interpolation == 'cubic':
        
        # peform cubic interpolation
        ...
        # peform quadratic interpolation
    ...
    # TODO: add an error if interpolation is set to something that isn't an option
   
    return alpha
    
def plot_linesearch(alpha1, alpha2, f1, f2, slope1, slope2, alpha, f_p, g_p, interpolation):
    ...
    if interpolation == 'cubic':
        ...
        y_pred = c0+c1*alpha+c2*(alpha**2)+c3*(alpha**3)

    elif interpolation == 'quadratic':
                
        y = [c0+c1*i+c2*(i**2) for i in x]
        y_pred = c0+c1*alpha+c2*(alpha**2)
       
    # plot predicted interpolation with actual    
    plt.figure()
    plt.plot(x,y)
    plt.plot([lower, alpha, upper], [f_lower, y_pred, f_upper],'o')
    plt.plot(alpha, f_p, 'x', color='red')
    plt.grid()
